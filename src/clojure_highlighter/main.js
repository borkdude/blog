...
import { tags, styleTags, defaultHighlightStyle, highlightTree } from "@codemirror/highlight";
...
document.querySelectorAll("code.clojure").forEach( elt => {
  let state = EditorState.create({doc: elt.innerText, extensions: [extensions]});
  console.log(state);
  console.log(state.doc.toString());
  highlightTree(state.tree);
  // console.log(highlight(elt.innerText));
